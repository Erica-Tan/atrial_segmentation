...
class UpConv(nn.Module):
    def __init__(self, spatial_dims: int, in_channels: int, out_channels: int, act: str | tuple=Act.PRELU,
                 norm: str | tuple=Norm.INSTANCE, kernel_size=3, strides=2, dropout=0.0):
        super().__init__()
        self.up = Convolution(
            spatial_dims,
            in_channels,
...
class AttentionBlock(nn.Module):
    def __init__(self, spatial_dims: int, f_int: int, f_g: int, f_l: int, dropout=0.0, act: str | tuple=Act.PRELU,
                 norm: str | tuple=Norm.INSTANCE):
        super().__init__()
...
class AttentionLayer(nn.Module):
    def __init__(
        ...
        self.merge = Convolution(
            spatial_dims=spatial_dims, in_channels=2 * in_channels, out_channels=in_channels, dropout=dropout, kernel_size=up_kernel_size
        )
...
class AttentionUnet(nn.Module):
    ...
    def __init__(
        ...
        head = ConvBlock(spatial_dims=spatial_dims, in_channels=in_channels, out_channels=channels[0], dropout=dropout, kernel_size=kernel_size)

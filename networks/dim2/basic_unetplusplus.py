...
class TwoConv(nn.Sequential):
    ...
    def __init__(
        ...
        conv_0 = Convolution(
            spatial_dims, in_chns, out_chns, act=act, norm=norm, dropout=dropout,
            bias=bias, padding=None, kernel_size=kernel_size, strides=strides
        )
        conv_1 = Convolution(
            spatial_dims, out_chns, out_chns, act=act, norm=norm, dropout=dropout,
            bias=bias, padding=None, kernel_size=kernel_size
        )
...
class UpCat(nn.Module):
    ...
    def __init__(
        ...
        self.convs = TwoConv(spatial_dims, cat_chns + up_chns, out_chns, act, norm, bias, dropout, kernel_size=kernel_size)
        self.is_pad = is_pad
...
class BasicUNetPlusPlus(nn.Module):
    def __init__(
        ...
        self.conv_0_0 = TwoConv(spatial_dims, in_channels, fea[0], act, norm, bias, dropout, kernel_size=kernel_size)
        self.conv_1_0 = TwoConv(spatial_dims, fea[0], fea[1], act, norm, bias, dropout, strides=2, kernel_size=kernel_size)
        self.conv_2_0 = TwoConv(spatial_dims, fea[1], fea[2], act, norm, bias, dropout, strides=2, kernel_size=kernel_size)
        self.conv_3_0 = TwoConv(spatial_dims, fea[2], fea[3], act, norm, bias, dropout, strides=2, kernel_size=kernel_size)
        self.conv_4_0 = TwoConv(spatial_dims, fea[3], fea[4], act, norm, bias, dropout, strides=2, kernel_size=kernel_size)

        self.upcat_0_1 = UpCat(spatial_dims, fea[1], fea[0], fea[0], act, norm, bias, dropout, upsample, halves=False, kernel_size=kernel_size)
        self.upcat_1_1 = UpCat(spatial_dims, fea[2], fea[1], fea[1], act, norm, bias, dropout, upsample, kernel_size=kernel_size)
        self.upcat_2_1 = UpCat(spatial_dims, fea[3], fea[2], fea[2], act, norm, bias, dropout, upsample, kernel_size=kernel_size)
        self.upcat_3_1 = UpCat(spatial_dims, fea[4], fea[3], fea[3], act, norm, bias, dropout, upsample, kernel_size=kernel_size)

        self.upcat_0_2 = UpCat(
            spatial_dims, fea[1], fea[0] * 2, fea[0], act, norm, bias, dropout, upsample, halves=False, kernel_size=kernel_size
        )
        self.upcat_1_2 = UpCat(spatial_dims, fea[2], fea[1] * 2, fea[1], act, norm, bias, dropout, upsample, kernel_size=kernel_size)
        self.upcat_2_2 = UpCat(spatial_dims, fea[3], fea[2] * 2, fea[2], act, norm, bias, dropout, upsample, kernel_size=kernel_size)

        self.upcat_0_3 = UpCat(
            spatial_dims, fea[1], fea[0] * 3, fea[0], act, norm, bias, dropout, upsample, halves=False, kernel_size=kernel_size
        )
        self.upcat_1_3 = UpCat(spatial_dims, fea[2], fea[1] * 3, fea[1], act, norm, bias, dropout, upsample, kernel_size=kernel_size)

        self.upcat_0_4 = UpCat(
            spatial_dims, fea[1], fea[0] * 4, fea[5], act, norm, bias, dropout, upsample, halves=False, kernel_size=kernel_size
        )
